...
Directory map — this repo ships one plugin, so the eval tree is flat: [contract/](contract/) — contract tests and fixtures (layer 1; own README); [routing-impact-study/](routing-impact-study/) — the routing-impact study protocol for the tier-policy outcome study, written for pre-registration but still a draft: not yet registered, by its own status line; [scenarios.md](scenarios.md) — tier-assignment scenarios (layer 3). The steering-claude-code skill's application scenarios live in that plugin's own repo: [steering-claude-code/evals/scenarios.md](https://github.com/TheRealBillSiegler/steering-claude-code/blob/main/evals/scenarios.md).
...
**Growth rule:** stated once in [the contract README](contract/README.md#growth-rule) — no fix without its regression case.
...
- [steering-claude-code/evals/scenarios.md](https://github.com/TheRealBillSiegler/steering-claude-code/blob/main/evals/scenarios.md) — lives in that plugin's own repo; six scenarios converted from the source article's own when-to-use examples and anti-patterns (Zod rule, never-push-to-main, release checklist, personal preferences, noisy dependency audit, monorepo orientation), plus one probing the skill's building-enforcement guidance (workflow-spawn enforcement), which exists in no article.
- `scenarios.md` (this repo) — one probe per ladder rung or durable rule in the skill's own contract, including the rule that "top tier" means the most capable model in the current session rather than a fixed name, and the rule that a model is written explicitly even when it matches the session default.
...
- No component verdict is valid until the harness records a positive control (a known load-bearing component recovered as load-bearing) and a placebo (an irrelevant component returning ceremony) — the calibration ticket ([#13](https://github.com/TheRealBillSiegler/siegler-plugins/issues/13)) blocks verdicts for exactly this reason.
...
| Skill bodies | capability-verified, necessity untested | the scenario baselines ([scenarios.md](scenarios.md), [steering-claude-code/evals/scenarios.md](https://github.com/TheRealBillSiegler/steering-claude-code/blob/main/evals/scenarios.md)) prove guidance is retrievable, not that tier choices differ without it | judgment probes with/without skill access |
